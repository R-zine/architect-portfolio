...
class Building implements GalleryItem {
  ...
  constructor(source: string, path: string, desc?: LocalizedText) {
    const asset = getGalleryAsset(source);
    this.source = source;
    this.img = asset.src;
    this.full = asset.full;
    this.srcSet = asset.srcSet;
...
const chervenaStena = [
  new Building("./img/chervenaStena2/червена стена 2 1.jpg", "none"),
  new Building("./img/chervenaStena2/червена стена 2 2.jpg", "none"),
];
...
const rootCategoryLabels: Record<string, LocalizedText> = {
  buildings: ["Архитектура", "Buildings"],
  interior: ["Интериори", "Interior Design"],
  furniture: ["Мебели", "Furniture"],
};

export function getCategoryLabel(category: string, language: Language): string {
  const rootLabel = rootCategoryLabels[category];
  if (rootLabel) return rootLabel[language];
  const parent = Object.values(pictureData)
    .flat()
    .find((item) => item.path === category && item.desc);
  return parent?.desc?.[language] ?? "";
}
